...
from six.moves import \
    input  # solves compatibility issue bw pyuthon 2.x and 3 for raw input that does exists in python 3
...
class LabAdmittanceController(BaseControllerFixed):
    ...
    def applyForce(self):
        ...
        try:
            self.apply_body_wrench(body_name="ur5::wrist_3_link", reference_frame=reference_frame,
    ...
    def updateKinematicsDynamics(self):
        ...
        # this is expressed in a workframe with the origin attached to the base frame origin
    ...
    def estimateContactForces(self):
        ...
        if self.use_torque_control:
            self.contactForceW = np.linalg.inv(self.J6.T).dot(self.h[:6] - self.tau[:6])[:3]
    ...
    def send_joint_trajectory(self):
        ...
        trajectory_client = actionlib.SimpleActionClient(
            "{}/follow_joint_trajectory".format("/" + self.robot_name + "/" + self.active_controller),
    ...
    def plotStuff(self):
        if not (conf.robot_params[p.robot_name]['control_mode'] == "trajectory"):
            if (lab_conf.admittance_control):
                plotJoint('position', 0, self.time_log, self.q_log, self.q_des_log, self.qd_log, self.qd_des_log, None,
                          None, self.tau_log,
                          self.tau_ffwd_log, self.joint_names, self.q_des_adm_log)
                plotAdmittanceTracking(3, self.time_log, self.x_ee_log, self.x_ee_des_log, self.x_ee_des_adm_log,
                                       self.contactForceW_log)
            else:
                plotJoint('position', 0, self.time_log, self.q_log, self.q_des_log, self.qd_log, self.qd_des_log, None,
                          None, self.tau_log,
                          self.tau_ffwd_log, self.joint_names, self.q_des_log)
            plotJoint('torque', 2, self.time_log, self.q_log, self.q_des_log, self.qd_log, self.qd_des_log, None, None,
                      self.tau_log,
